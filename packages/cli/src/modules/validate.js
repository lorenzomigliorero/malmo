const fs = require('fs');
const { emitter } = require('@malmo/cli-utils');
...
module.exports = () => {
  ...
  if (!existsValidOption && (_unknown || (!commands.includes(command)))) {
    emitter.emit('error', new Error(labels['error.command']({ command: `${_unknown || command}` })));
  }
  ...
  if (!existsValidOption && command === 'init') {
    ...
    if (
      ...
      emitter.emit('error', new Error(labels['error.configurationAlreadyExist']()));
    }
  }
  ...
  if (!existsValidOption && command !== 'init') {
    ...
    if (
      ...
      emitter.emit('error', new Error(labels['error.configuration']({ configFileName })));
    }
  ...
  if (!existsValidOption && env) {
    ...
    if (
      ...
      emitter.emit('error', new Error(labels['error.invalidEnv']({ env })));
    }
  }
};
